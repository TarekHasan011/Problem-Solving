...
const int MOD = 1e9+7;
int euclidean_distance(pair< int , int > x, pair< int , int > y)
{
    return ((x.first-y.first)*(x.first-y.first)) + ((x.second-y.second)*(x.second-y.second));
}
int blust(vector< int > &temp, int r)
{
    int left = 0;
...
int main()
{
    ...
    while(cin >> n >> m && n && m)
    {
        ...
        for(int i=0;i<n;i++)
        {
            ...
            for(int j=0;j<n;j++)
            {
                temp.push_back(euclidean_distance(v[i],v[j]));
            }
        ...
        while(m--)
        {
            ...
            for(int i=0;i<n;i++)
            {
                final_result += (p*blust(check[i],r));
            }
