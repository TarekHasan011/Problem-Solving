...
int extract_max_load(map<string,vector<pair<string,int> > > adjList,string parent,string current_node,string destination,int min_so_far){
...
//    cout<<"P: "<<parent<<" C: "<<current_node<<" D: "<<destination<<" M: "<<min_so_far<<endl;
    ...
    for(pair<string,int> e : adjs){
        ...
        int m = extract_max_load(adjList,current_node,e.first,destination,t_m);
...
//    read_auto();
    ...
    while(cin>>n>>r && n+r!=0){
        ...
        int max_load = extract_max_load(adjList,"<no parent>",s,t,edges[0].first);
